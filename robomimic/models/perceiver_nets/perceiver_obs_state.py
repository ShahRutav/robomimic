...
class PerceiverVisionStateTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        if use_video:
            ...
            self.video_dim = [max_frames, (img_size//patch_size)**2+1] ## video and img are of same size
            self.latent_size = latent_size_n
        ...
        if use_text and use_decoder:

            self.mlm_pos = nn.Embedding(max_text_len, embed_dim)
        ...
        if use_robot_state:
            ...
            self.robot_embed = nn.Linear(robot_state_dim, embed_dim)
            self.robot_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))

        self.latents = nn.Parameter(torch.randn(latent_size_n, embed_dim))
    ...
    def robot_state_embed(self, state, mask_it=False):
        assert state.shape[-1] == self.robot_state_dim
        if len(state.size()) == 3:
            use_video = True
        else:
            use_video = False
        ...
        B, T, _ = state.shape
        x = self.robot_embed(state)
        if use_video: ## First add temporal embedding and then classification token
            temporal_embed = self.temporal_embed[:,:T,:].expand(B, -1, -1)
            assert x.shape == temporal_embed.shape, print("x.shape {}, temporal_embed.shape{}".format(x.shape, temporal_embed.shape)) ## no funny business
            x += temporal_embed
    ...
    def forward_unimo(self, inputs, masks):
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    ...
    """
    model = PerceiverVisionStateTransformer(
        embed_dim=768,
        patch_size=16,
        img_size=224,
        depth=12,
